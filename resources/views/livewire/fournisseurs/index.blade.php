        ...
        @else
                        ...
                        <tbody class="divide-y divide-slate-700">
                            @foreach ($fournisseurs as $f)
                                <tr class="hover:bg-slate-700/30 transition-colors">
                                    <td class="px-4 py-3 font-mono text-blue-400">{{ $f->code_fournisseur }}</td>
                                    <td class="px-4 py-3">
                                        @if($f->type_personne == 'PHYSIQUE')
                                            {{ $f->prenom }} {{ $f->nom }}
                                        @else
                                    ...
                                    <td class="px-4 py-3">
                                        @if($f->telephone)<div>{{ $f->telephone }}</div>@endif
                                        @if($f->whatsapp)<div class="text-green-400 text-xs">WhatsApp: {{ $f->whatsapp }}</div>@endif
                                        @if($f->email)<div class="text-blue-400 text-xs">{{ $f->email }}</div>@endif
                                    </td>
                                    <td class="px-4 py-3">{{ $f->localite?->nom ?? '-' }}</td>
                                    <td class="px-4 py-3">{{ $f->type_fournisseur }}</td>
                                    <td class="px-4 py-3">
                                        <div class="flex items-center justify-center gap-1.5">
                                            <button wire:click="show({{ $f->id }})" class="p-1.5 bg-blue-600/90 hover:bg-blue-500 text-white rounded-md transition" title="Voir">
                                                👁️
                                            </button>
                                            <button wire:click="edit({{ $f->id }})" class="p-1.5 bg-yellow-600/90 hover:bg-yellow-500 text-white rounded-md transition" title="Modifier">
                                                ✏️
                                            </button>
                                            <button wire:click="delete({{ $f->id }})" wire:confirm="Supprimer ce fournisseur ?" class="p-1.5 bg-red-600/90 hover:bg-red-500 text-white rounded-md transition" title="Supprimer">
                                                🗑️
                                            </button>
                                        </div>
                                    </td>
            ...
            <div class="mt-6 flex items-center justify-between">
                <div class="text-sm text-slate-400">
                    {{ ($fournisseurs->currentPage() - 1) * $fournisseurs->perPage() + 1 }} - {{ min($fournisseurs->currentPage() * $fournisseurs->perPage(), $fournisseurs->total()) }} sur {{ $fournisseurs->total() }}
                </div>
                <div>{{ $fournisseurs->links() }}</div>
            </div>
        @endif
                ...
                <div class="bg-slate-800 border border-slate-700 rounded-xl shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
                    <div class="p-6">
                        <h3 class="text-xl font-semibold text-white mb-4">
                            {{ $viewMode ? 'Détails du fournisseur' : ($form['id'] ? 'Modifier le fournisseur' : 'Nouveau fournisseur') }}
                        </h3>

                        @if($viewMode)
                            {{-- Mode visualisation --}}
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                <div><label class="block text-xs text-slate-400">Code</label><p class="text-white">{{ $form['code_fournisseur'] }}</p></div>
                                <div><label class="block text-xs text-slate-400">Type personne</label><p class="text-white">{{ $form['type_personne'] }}</p></div>
                                @if($form['type_personne'] == 'PHYSIQUE')
                                    <div><label class="block text-xs text-slate-400">Nom</label><p class="text-white">{{ $form['nom'] }}</p></div>
                                    <div><label class="block text-xs text-slate-400">Prénom</label><p class="text-white">{{ $form['prenom'] }}</p></div>
                                @else
                                    <div><label class="block text-xs text-slate-400">Raison sociale</label><p class="text-white">{{ $form['raison_sociale'] }}</p></div>
                                    <div><label class="block text-xs text-slate-400">Sigle</label><p class="text-white">{{ $form['sigle'] }}</p></div>
                                @endif
                                <div><label class="block text-xs text-slate-400">Téléphone</label><p class="text-white">{{ $form['telephone'] ?? '-' }}</p></div>
                                <div><label class="block text-xs text-slate-400">WhatsApp</label><p class="text-white">{{ $form['whatsapp'] ?? '-' }}</p></div>
                                <div><label class="block text-xs text-slate-400">Email</label><p class="text-white">{{ $form['email'] ?? '-' }}</p></div>
                                <div><label class="block text-xs text-slate-400">Localité</label><p class="text-white">{{ $localites->firstWhere('id', $form['localite_id'])?->nom ?? '-' }}</p></div>
                                <div><label class="block text-xs text-slate-400">Type fournisseur</label><p class="text-white">{{ $form['type_fournisseur'] ?? '-' }}</p></div>
                            </div>
                            ...
                            <form wire:submit.prevent="save" class="space-y-4">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        @error('form.type_personne') <span class="text-xs text-red-400">{{ $message }}</span> @enderror
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        @error('form.code_fournisseur') <span class="text-xs text-red-400">{{ $message }}</span> @enderror
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <select wire:model="form.localite_id" class="mt-1 block w-full rounded-md border-slate-600 bg-slate-700 text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                                            ...
                                            @foreach($localites as $localite)
                                                <option value="{{ $localite->id }}">{{ $localite->nom }} ({{ $localite->region }})</option>
                                            @endforeach
                                        </select>
                                    </div>
