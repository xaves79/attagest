...
<body>
    ...
    <div class="kpis">
        <div class="kpi amber">
            ...
            <div class="value">{{ $achats->nb ?? 0 }}</div>
        </div>
        <div class="kpi amber">
            <div class="label">Quantité totale</div>
            <div class="value">{{ number_format($achats->total_kg ?? 0, 0, ',', ' ') }}</div>
            <div class="unit">kg</div>
        </div>
        <div class="kpi amber">
            <div class="label">Montant total</div>
            <div class="value">{{ number_format($achats->total_fcfa ?? 0, 0, ',', ' ') }}</div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $f->nom }}</td>
                <td class="right">{{ $f->nb_achats }}</td>
                <td class="right amber">{{ number_format($f->total_kg, 0, ',', ' ') }}</td>
                <td class="right">{{ number_format($f->total_fcfa, 0, ',', ' ') }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <div class="kpis">
        <div class="kpi blue">
            ...
            <div class="value">{{ number_format($etuvages->total_entree ?? 0, 0, ',', ' ') }}</div>
            <div class="unit">kg · {{ $etuvages->nb ?? 0 }} étuvage(s)</div>
        </div>
        <div class="kpi blue">
            <div class="label">Riz étuvé produit</div>
            <div class="value">{{ number_format($rizEtuve->total_kg ?? 0, 0, ',', ' ') }}</div>
            <div class="unit">kg · Rdt : {{ number_format($rizEtuve->rdt_moy ?? 0, 1) }}%</div>
        </div>
        <div class="kpi green">
            <div class="label">Riz blanc produit</div>
            <div class="value">{{ number_format($decorticages->total_blanc ?? 0, 0, ',', ' ') }}</div>
            <div class="unit">kg · {{ $decorticages->nb ?? 0 }} décorticage(s)</div>
        </div>
        <div class="kpi">
            <div class="label">Rendement décorti.</div>
            <div class="value">{{ number_format($decorticages->rdt_moy ?? 0, 1) }}%</div>
            <div class="unit">Son : {{ number_format($decorticages->total_son ?? 0, 0, ',', ' ') }} kg</div>
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $v->variete }}</td>
                <td class="right green">{{ number_format($v->riz_blanc_kg, 0, ',', ' ') }}</td>
                <td class="right amber">{{ number_format($v->son_kg, 0, ',', ' ') }}</td>
                <td class="right">{{ number_format($v->rdt_moy, 1) }}%</td>
            </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <div class="kpis">
        <div class="kpi green">
            ...
            <div class="value">{{ $commandes->nb ?? 0 }}</div>
            <div class="unit">{{ $commandes->nb_comptant ?? 0 }} comptant · {{ $commandes->nb_credit ?? 0 }} crédit</div>
        </div>
        <div class="kpi green">
            <div class="label">CA total</div>
            <div class="value">{{ number_format($commandes->total_fcfa ?? 0, 0, ',', ' ') }}</div>
            <div class="unit">FCFA</div>
        </div>
        <div class="kpi green">
            <div class="label">Encaissé clients</div>
            <div class="value">{{ number_format($paiementsClients->total_encaisse ?? 0, 0, ',', ' ') }}</div>
            <div class="unit">FCFA · {{ $paiementsClients->nb ?? 0 }} paiement(s)</div>
        </div>
        <div class="kpi {{ ($soldesClients->total_solde ?? 0) > 0 ? 'red' : 'green' }}">
            <div class="label">Soldes clients dus</div>
            <div class="value">{{ number_format($soldesClients->total_solde ?? 0, 0, ',', ' ') }}</div>
            <div class="unit">FCFA · {{ $soldesClients->nb_factures ?? 0 }} facture(s)</div>
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $c->raison_sociale ?: $c->nom }}</td>
                <td class="right">{{ $c->nb_commandes }}</td>
                <td class="right green">{{ number_format($c->total_fcfa, 0, ',', ' ') }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <div class="kpis">
        ...
        <div class="kpi red">
            ...
            <div class="value">- {{ number_format($paiementsFournisseurs->total_paye ?? 0, 0, ',', ' ') }}</div>
            <div class="unit">FCFA</div>
        </div>
        <div class="kpi {{ ($soldesFournisseurs->total_solde ?? 0) > 0 ? 'amber' : 'green' }}">
            <div class="label">Solde dû fournisseurs</div>
            <div class="value">{{ number_format($soldesFournisseurs->total_solde ?? 0, 0, ',', ' ') }}</div>
            <div class="unit">FCFA · {{ $soldesFournisseurs->nb_recus ?? 0 }} reçu(s)</div>
        </div>
    </div>

    @php $flux = ($paiementsClients->total_encaisse ?? 0) - ($paiementsFournisseurs->total_paye ?? 0); @endphp
    <div class="flux-box" style="{{ $flux < 0 ? 'background:#fef2f2;border-color:#fca5a5;' : '' }}">
        <div class="flux-label" style="{{ $flux < 0 ? 'color:#dc2626;' : '' }}">Flux net de trésorerie sur la période</div>
        <div class="flux-value" style="{{ $flux < 0 ? 'color:#dc2626;' : '' }}">
            {{ $flux >= 0 ? '+' : '' }}{{ number_format($flux, 0, ',', ' ') }} FCFA
        </div>
    </div>

    {{-- PIED DE PAGE --}}
    <div class="footer">
        <span>{{ $entreprise->nom ?? 'ATTAGEST SARL' }} — {{ $entreprise->adresse ?? 'Bouaké, Côte d\'Ivoire' }}</span>
