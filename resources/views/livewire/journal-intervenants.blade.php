    ...
    <div class="max-w-7xl mx-auto px-6 py-8 space-y-6">
        ...
        <div class="grid grid-cols-3 md:grid-cols-6 gap-3">
            ...
            <div class="bg-slate-900 border border-slate-700/60 rounded-xl p-3 text-center">
                <p class="text-lg mb-1">{{ $cfg[0] }}</p>
                <p class="text-xl font-black {{ $cfg[1] }}">{{ $stats->get($action)?->nb ?? 0 }}</p>
                <p class="text-xs text-slate-500">{{ $cfg[2] }}</p>
            </div>
            @endforeach
        </div>
        ...
        <div class="bg-slate-900 border border-slate-700/60 rounded-2xl overflow-hidden">
                ...
                <tbody class="divide-y divide-slate-800">
                    ...
                    <tr class="hover:bg-slate-800/40 transition">
                        <td class="px-5 py-3 text-xs text-slate-500">
                            ...
                            <span class="text-slate-600">{{ \Carbon\Carbon::parse($a->created_at)->format('H:i:s') }}</span>
                        </td>
                        <td class="px-5 py-3">
                            <p class="text-slate-300 text-xs font-semibold">{{ $a->user_name ?? 'Système' }}</p>
                            <p class="text-xs {{ $roleBadge[$a->user_role] ?? 'text-slate-500' }}">{{ ucfirst($a->user_role) }}</p>
                        </td>
                        <td class="px-5 py-3">
                            <span class="px-2 py-0.5 rounded-full text-xs border {{ $actionBadge[$a->action] ?? 'bg-slate-800 text-slate-400 border-slate-700' }}">
                                {{ ucfirst($a->action) }}
                            </span>
                        </td>
                        <td class="px-5 py-3">
                            <span class="text-xs font-mono text-cyan-400">{{ $a->module }}</span>
                        </td>
                        <td class="px-5 py-3 text-slate-300 text-xs">{{ $a->description }}</td>
                        <td class="px-5 py-3 text-xs text-slate-600 font-mono">{{ $a->ip_address }}</td>
                    </tr>
                    @empty
            ...
            <div class="px-5 py-4 border-t border-slate-800">{{ $activites->links() }}</div>
            @endif
        </div>
    </div>
</div>