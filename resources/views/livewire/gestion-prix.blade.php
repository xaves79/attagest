<div>
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-gray-100 dark:bg-gray-800 rounded-lg p-6 mb-8 shadow">
            <h3 class="text-lg font-semibold mb-4">{{ $formMode === 'create' ? 'Ajouter un prix' : 'Modifier le prix' }}</h3>
            <form wire:submit.prevent="save">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        @error('type_produit') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('poids_sac_kg') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
                    @endif
                    ...
                    <div>
                        ...
                        @error('prix_unitaire_fcfa') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('date_application') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
        ...
        <div class="bg-white dark:bg-gray-800 shadow overflow-hidden sm:rounded-lg">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($prix as $p)
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $p->type_produit }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $p->unite }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $p->poids_sac_kg ?: '—' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ number_format($p->prix_unitaire_fcfa, 0, ',', ' ') }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ \Carbon\Carbon::parse($p->date_application)->format('d/m/Y') }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $p->actif ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                {{ $p->actif ? 'Oui' : 'Non' }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <button wire:click="edit({{ $p->id }})" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 mr-3">Modifier</button>
                            <button wire:click="delete({{ $p->id }})" onclick="return confirm('Supprimer ce prix ?')" class="text-red-600 hover:text-red-900 dark:text-red-400">Supprimer</button>
                        </td>
                    </tr>
                    @empty
                    <tr>
