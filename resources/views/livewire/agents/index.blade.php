<div>
    ...
    @else
                    ...
                    <tbody class="divide-y divide-gray-700">
                        @foreach ($agents as $a)
                            <tr class="hover:bg-gray-800/50 transition-all duration-200 group">
                                <td class="px-6 py-4">
                                    <img src="{{ $a->photo_url }}" alt="Photo" class="w-10 h-10 rounded-full object-cover">
                                </td>
                                <td class="px-6 py-4 font-mono">{{ $a->matricule }}</td>
                                <td class="px-6 py-4">
                                    {{ $a->nom_complet ?? ($a->prenom . ' ' . $a->nom) }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $a->poste?->libelle ?? '-' }}
                                </td>
                                ...
                                <td class="px-6 py-4 text-sm">
                                    ...
                                    @endif
                                    @if($a->whatsapp)
                                        <div class="text-green-400">WhatsApp: {{ $a->whatsapp }}</div>
                                    @endif
                                    @if($a->email)
                                        <div class="text-blue-400">{{ $a->email }}</div>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
        ...
        <div class="mt-8 flex justify-between items-center text-sm text-gray-400">
            <div>
                {{ ($agents->currentPage() - 1) * $agents->perPage() + 1 }} -
                {{ min($agents->currentPage() * $agents->perPage(), $agents->total()) }}
                sur {{ $agents->total() }}
            </div>
        ...
        <div class="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
            <div class="bg-gray-900 border border-gray-700 rounded-lg shadow-xl max-w-4xl w-full p-6 mx-4 text-white max-h-[90vh] overflow-y-auto">
                <h2 class="text-xl font-semibold mb-4">
                    {{ $viewMode ? 'Détails de l\'agent' : ($form['id'] ? 'Modifier l\'agent' : 'Nouvel agent') }}
                </h2>
                ...
                @if($viewMode)
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <p class="mt-1">{{ $form['date_embauche'] ? \Carbon\Carbon::parse($form['date_embauche'])->format('d/m/Y') : '-' }}</p>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-400">Actif</label>
                            <p class="mt-1">{{ $form['actif'] ? 'Oui' : 'Non' }}</p>
                        </div>
                        <div class="col-span-2">
                            <label class="block text-xs font-medium text-gray-400">Nom complet (affiché)</label>
                            <p class="mt-1">{{ $form['nom_complet'] ?? ($form['prenom'] . ' ' . $form['nom']) }}</p>
                        </div>
                    </div>
                ...
                @else
                    ...
                    <form wire:submit.prevent="save" enctype="multipart/form-data">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            {{-- Champ photo avec indicateur de chargement --}}
                            <div>
                                ...
                                @if ($photo)
                                    <div class="mt-2">
                                        <img src="{{ $photo->temporaryUrl() }}" class="h-20 rounded-lg">
                                    </div>
                                @elseif ($form['photo'])
                                    <div class="mt-2">
                                        <img src="{{ asset('storage/' . $form['photo']) }}" class="h-20 rounded-lg">
                                    </div>
                                @endif
                            </div>
                        ...
                        <div class="mt-6 flex justify-end space-x-3">
                            ...
                            {{-- Bouton de soumission désactivé pendant l'upload --}}
                            <button
                                type="submit"
                                wire:loading.attr="disabled"
