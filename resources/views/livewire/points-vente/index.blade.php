<div>
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            ...
            <p class="text-gray-400 mt-1">{{ $points->total() }} point{{ $points->total() > 1 ? 's' : '' }} de vente trouvé{{ $points->total() > 1 ? 's' : '' }}</p>
        </div>
        <button
            wire:click="create"
    ...
    @else
                    ...
                    <tbody class="divide-y divide-gray-700">
                        @foreach ($points as $p)
                            <tr class="hover:bg-gray-800/50 transition-all duration-200 group">
                                ...
                                <td class="px-6 py-4 font-mono">{{ $p->code_point }}</td>
                                <td class="px-6 py-4">
                                    {{ $p->agent?->prenom ? $p->agent->prenom . ' ' : '' }}{{ $p->agent?->nom ?? '-' }}
                                </td>
                                <td class="px-6 py-4">{{ $p->localite?->nom ?? '-' }}</td>
        ...
        <div class="mt-8 flex justify-between items-center text-sm text-gray-400">
            <div>
                {{ ($points->currentPage() - 1) * $points->perPage() + 1 }} - {{ min($points->currentPage() * $points->perPage(), $points->total()) }} sur {{ $points->total() }}
            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <select wire:model="form.agent_id" class="mt-1 block w-full rounded-md border-gray-600 bg-gray-800 text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                                ...
                                @foreach($agents as $a)
                                    <option value="{{ $a->id }}">{{ $a->prenom ? $a->prenom . ' ' : '' }}{{ $a->nom }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
