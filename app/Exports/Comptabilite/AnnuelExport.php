...
class AnnuelExport implements FromCollection, WithHeadings, WithMapping, WithStyles
{
    protected $debut;
    protected $fin;
    ...
    {
        $now = now();
        $this->debut = $now->startOfYear()->format('Y-m-d');
        $this->fin   = $now->endOfYear()->format('Y-m-d');
    }
