        ...
        <div class="max-w-7xl mx-auto">
            @if (session()->has('message'))
                <div class="bg-emerald-800/90 border border-emerald-600/50 backdrop-blur-sm text-emerald-200 px-6 py-4 rounded-xl mb-6 animate-pulse shadow-lg">
                    <i class="fas fa-check-circle mr-3"></i>{{ session('message') }}
                </div>
            @endif
                        ...
                        <div class="bg-gradient-to-r from-gray-800/70 to-gray-900/50 border border-gray-700/50 rounded-2xl p-6 backdrop-blur-sm shadow-2xl">
                            <div class="flex justify-between items-center mb-6">
                                <h3 class="text-2xl font-bold text-emerald-400 flex items-center">
                                    ...
                                    Factures récentes ({{ $facturesRecentes->count() }})
                                </h3>
                                            ...
                                            <tr class="hover:bg-gray-700/50 transition-all border-b border-gray-800/30 group">
                                                ...
                                                <td class="py-4 px-4 text-center">
                                                    <span class="px-4 py-2 rounded-full text-xs font-bold inline-block shadow-lg
                                                        {{ $facture->statut == 'paye' ? 'bg-emerald-500/20 text-emerald-300 border-emerald-400/50' : 'bg-orange-500/20 text-orange-300 border-orange-400/50' }}
                                                        border-2">
                                                        {{ $facture->statut == 'paye' ? '✅ Payée' : '⏳ Crédit' }}
                                                    </span>
                                                </td>
                        ...
                        <form wire:submit.prevent="save" class="p-8 space-y-8 overflow-y-auto max-h-[calc(95vh-200px)]">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                <div class="md:col-span-2">
                                    ...
                                    @error('form.point_vente_id') 
                                        <span class="text-red-400 text-sm mt-2 block font-semibold">{{ $message }}</span> 
                                    @enderror
                                </div>
                            ...
                            <div class="bg-gradient-to-r from-gray-800/70 to-gray-900/50 border-2 border-gray-700/50 rounded-3xl p-8 shadow-2xl">
                                ...
                                <div class="space-y-4 max-h-96 overflow-y-auto pr-4 -mr-4 pb-4">
                                    @forelse($lignes as $index => $ligne)
                                        <div wire:key="ligne-{{ $index }}" class="group bg-gray-800/50 hover:bg-gray-700/50 border-2 border-gray-700/50 hover:border-emerald-500/30 rounded-2xl p-6 transition-all hover:shadow-xl">
                                            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-end">
                                                <div class="lg:col-span-4">
                                                    <label class="block text-sm font-semibold text-gray-300 mb-3">Article *</label>
                                                    <select wire:model.live="lignes.{{ $index }}.article_id" class="w-full bg-gray-900/50 border border-gray-600/50 rounded-xl px-4 py-3 text-base focus:ring-2 focus:ring-emerald-500">
                                                        ...
                                                        @foreach($articles as $article)
                                                            ...
                                                            @endphp
                                                            <option value="{{ $article->id }}" {{ ($stock <= 0 && !$isSelected) ? 'disabled' : '' }}>
                                                                {{ $article->nom }}
                                                                <span class="ml-2 px-2 py-1 rounded text-xs font-bold
                                                                    {{ $stock > 0 ? 'bg-emerald-500/20 text-emerald-300' : 'bg-red-500/20 text-red-300' }}">
                                                ...
                                                <div class="lg:col-span-2">
													...
													<div class="relative">
														<input 
															...
															min="1" 
															max="{{ $qteMax }}"
															class="w-full bg-gray-900/50 border-2 {{ $isOverStock ? 'border-red-500/70 bg-red-500/20 ring-2 ring-red-400/50' : 'border-gray-600/50' }} rounded-xl px-6 py-4 text-xl font-bold focus:ring-4 focus:ring-emerald-500/30 shadow-lg pr-20 transition-all"
															placeholder="1-{{ $qteMax }}"
														>
														@if($stockDispo > 0)
															<div class="absolute right-3 top-1/2 -translate-y-1/2 text-sm font-bold {{ $isOverStock ? 'text-red-400 bg-red-900/80' : 'text-orange-400 bg-black/50' }} px-3 py-1 rounded-lg border">
																{{ $isOverStock ? 'Max: ' . $stockDispo : 'Stock: ' . $stockDispo }}
															</div>
														@else
                                                ...
                                                <div class="lg:col-span-3">
                                                    ...
                                                    <div class="w-full bg-gradient-to-r from-blue-900/70 to-blue-800/40 border border-blue-600/50 rounded-xl px-6 py-4 text-right text-xl font-bold text-blue-300 shadow-lg">
                                                        {{ number_format($ligne['prix_unitaire'] ?? 0, 0, ',', ' ') }} FCFA
                                                    </div>
                                                </div>
                                                ...
                                                <div class="lg:col-span-2">
                                                    ...
                                                    <div class="w-full bg-gradient-to-r from-emerald-900/80 to-emerald-800/50 border-2 border-emerald-600/50 rounded-xl px-6 py-4 text-right text-2xl font-black text-emerald-400 shadow-2xl">
                                                        {{ number_format($ligne['montant'] ?? 0, 0, ',', ' ') }} FCFA
                                                    </div>
                                                </div>

                                                <div class="lg:col-span-1 flex justify-end pt-10 lg:pt-0">
                                                    <button type="button" wire:click="removeLigne({{ $index }})" class="w-14 h-14 group-hover:bg-red-600 bg-red-500/80 hover:bg-red-600 text-white rounded-2xl flex items-center justify-center text-xl font-bold shadow-xl hover:shadow-2xl transition-all group-hover:scale-110 hover:rotate-12">
                            ...
                            <div class="bg-gradient-to-r from-emerald-900/90 to-yellow-900/70 border-4 border-emerald-500/60 rounded-3xl p-12 text-center shadow-2xl backdrop-blur-xl">
                                <div class="text-6xl font-black text-emerald-400 mb-6 animate-pulse drop-shadow-2xl">
                                    {{ number_format($form['montant_total'], 0, ',', ' ') }}
                                </div>
                                ...
                                <div class="text-2xl font-black text-emerald-300 bg-gradient-to-r from-emerald-500/40 to-yellow-500/30 px-12 py-6 rounded-2xl inline-block border-4 border-emerald-400/50 shadow-2xl">
                                    {{ number_format($form['montant_total'], 0, ',', ' ') }} FCFA
                                </div>
