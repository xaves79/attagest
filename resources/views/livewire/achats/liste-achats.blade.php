    ...
    <div class="max-w-7xl mx-auto px-6 py-8 space-y-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="bg-slate-900 border border-slate-700/60 rounded-2xl p-5">
                ...
                <p class="text-3xl font-black text-white">{{ $kpis['nb_lots'] }}</p>
            </div>
            <div class="bg-slate-900 border border-slate-700/60 rounded-2xl p-5">
            ...
            <div class="bg-slate-900 border border-slate-700/60 rounded-2xl p-5">
                ...
                <p class="text-2xl font-black text-amber-400">{{ number_format($kpis['total_montant'], 0, ',', ' ') }}<span class="text-sm font-normal text-slate-500 ml-1">FCFA</span></p>
            </div>
            <div class="bg-slate-900 border border-green-700/40 rounded-2xl p-5">
                ...
                <p class="text-3xl font-black text-green-400">{{ number_format($kpis['disponible_kg'], 0, ',', ' ') }}<span class="text-base font-normal text-slate-500 ml-1">kg</span></p>
            </div>
        </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-800">
                    ...
                    <tr class="hover:bg-slate-800/40 transition group">
                        <td class="px-5 py-3">
                            <span class="font-mono text-amber-400 text-xs font-bold">{{ $achat->code_lot }}</span>
                        </td>
                        <td class="px-5 py-3 text-slate-300">{{ $achat->fournisseur?->nom ?? '—' }}</td>
                        <td class="px-5 py-3 text-slate-400 text-xs">{{ $achat->variete?->nom ?? '—' }}</td>
                        <td class="px-5 py-3 text-slate-400 text-xs">{{ $achat->date_achat?->format('d/m/Y') }}</td>
                        <td class="px-5 py-3 text-right text-slate-300 font-semibold">
                            {{ number_format($achat->quantite_achat_kg, 0, ',', ' ') }} kg
                        </td>
                        <td class="px-5 py-3 text-right font-semibold
                            {{ $achat->quantite_restante_kg > 0 ? 'text-green-400' : 'text-slate-600' }}">
                            {{ number_format($achat->quantite_restante_kg, 0, ',', ' ') }} kg
                        </td>
                        ...
                        <td class="px-5 py-3 text-center">
                            ...
                            @endphp
                            <span class="px-2 py-0.5 rounded-full text-xs border {{ $badges[$achat->statut] ?? 'bg-slate-800 text-slate-400 border-slate-700' }}">
                                {{ ucfirst(str_replace('_', ' ', $achat->statut ?? '—')) }}
                            </span>
                        </td>
