<div class="max-w-6xl mx-auto px-4 py-8">
    <div class="bg-slate-800 border border-slate-700 rounded-xl p-6 space-y-6">
        ...
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold text-white">Commande {{ $commande->code_commande }}</h1>
                <div class="flex gap-2 mt-2">
                    <span class="px-3 py-1 text-xs font-medium rounded-full
                        {{ $commande->statut === 'livree' ? 'bg-green-900/50 text-green-300 border border-green-600' : 'bg-yellow-900/50 text-yellow-300 border border-yellow-600' }}">
                        🚚 {{ ucfirst($commande->statut) }}
                    </span>
                    <span class="px-3 py-1 text-xs font-medium rounded-full bg-blue-900/50 text-blue-300 border border-blue-600">
                        {{ ucfirst($commande->type_vente) }}
                    </span>
                </div>
            </div>
            <a href="{{ route('commandes.index') }}" class="text-slate-400 hover:text-white">← Retour à la liste</a>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-slate-700/50 rounded-lg p-4">
                ...
                <div class="space-y-1 text-slate-200">
                    <p><span class="text-slate-400">Commande :</span> {{ $commande->date_commande->format('d/m/Y') }}</p>
                    @if($commande->date_livraison_prevue)
                        <p><span class="text-slate-400">Livraison prévue :</span> {{ \Carbon\Carbon::parse($commande->date_livraison_prevue)->format('d/m/Y') }}</p>
                    @endif
                    @if($commande->date_livraison_effective)
                        <p><span class="text-slate-400">Livré le :</span> {{ \Carbon\Carbon::parse($commande->date_livraison_effective)->format('d/m/Y') }}</p>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-slate-700/50 rounded-lg p-4">
                ...
                <p class="text-slate-200">{{ $commande->pointVente->nom ?? '—' }}</p>
                <p class="text-slate-400 text-sm">Agent : {{ $commande->agent->prenom }} {{ $commande->agent->nom }}</p>
            </div>

            <div class="bg-slate-700/50 rounded-lg p-4">
                <h2 class="text-sm font-semibold text-slate-400 uppercase mb-2">👤 Client</h2>
                <p class="text-slate-200 font-medium">{{ $commande->client->nom }} {{ $commande->client->prenom }}</p>
                <p class="text-slate-400 text-sm">{{ $commande->client->code_client }}</p>
                <p class="text-slate-400 text-sm">{{ $commande->client->telephone }}</p>
                <p class="text-slate-400 text-sm">{{ $commande->client->ville ?? '—' }}</p>
            </div>

            <div class="bg-slate-700/50 rounded-lg p-4">
                <h2 class="text-sm font-semibold text-slate-400 uppercase mb-2">📝 Notes</h2>
                <p class="text-slate-200">{{ $commande->notes ?: 'Aucune note' }}</p>
            </div>
        </div>
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-700">
                        @foreach($commande->lignes as $ligne)
                            ...
                            <tr class="hover:bg-slate-700/30">
                                <td class="px-4 py-3">
                                    <div>{{ $ligne->sac->code_sac ?? $ligne->type_produit }}</div>
                                ...
                                <td class="px-4 py-3 text-center">{{ $ligne->quantite }} {{ $ligne->unite }}</td>
                                <td class="px-4 py-3 text-center">{{ $quantiteLivree }}</td>
                                <td class="px-4 py-3 text-center">{{ $reste }}</td>
                                <td class="px-4 py-3 text-right">{{ number_format($ligne->prix_unitaire_fcfa, 0, ',', ' ') }} FCFA</td>
                                <td class="px-4 py-3 text-right">{{ number_format($ligne->quantite * $ligne->prix_unitaire_fcfa, 0, ',', ' ') }} FCFA</td>
                            </tr>
                        @endforeach
                    </tbody>
                    <tfoot class="bg-slate-700">
                        <tr>
                            ...
                            <td class="px-4 py-3 text-right font-bold">{{ number_format($commande->montant_total_fcfa, 0, ',', ' ') }} FCFA</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
                ...
                <div class="space-y-2">
                    @foreach($commande->livraisons as $liv)
                        <div class="bg-slate-700/30 rounded p-3 flex justify-between items-center">
                            <div>
                                <span class="font-mono text-blue-400">{{ $liv->code_livraison }}</span>
                                <span class="text-slate-400 text-sm ml-2">{{ $liv->date_livraison->format('d/m/Y H:i') }}</span>
                                <div class="text-sm text-slate-300">Agent : {{ $liv->agent->prenom }} {{ $liv->agent->nom }}</div>
                            </div>
                            <div class="text-sm">
                                @foreach($liv->lignes as $ll)
                                    <div>{{ $ll->quantite }} × {{ $ll->stockSac->sac->code_sac ?? '—' }}</div>
                                @endforeach
                            </div>
                        </div>
                    @endforeach
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div class="bg-slate-700/50 rounded-lg p-4 text-center">
                    ...
                    <p class="text-xl font-bold text-white">{{ number_format($commande->montant_total_fcfa, 0, ',', ' ') }} FCFA</p>
                </div>
                <div class="bg-slate-700/50 rounded-lg p-4 text-center">
                    <p class="text-slate-400 text-sm">Payé</p>
                    <p class="text-xl font-bold text-green-400">{{ number_format($commande->montant_acompte_fcfa, 0, ',', ' ') }} FCFA</p>
                </div>
                <div class="bg-slate-700/50 rounded-lg p-4 text-center">
                    <p class="text-slate-400 text-sm">Solde</p>
                    <p class="text-xl font-bold {{ $commande->montant_total_fcfa - $commande->montant_acompte_fcfa > 0 ? 'text-yellow-400' : 'text-green-400' }}">
                        {{ number_format($commande->montant_total_fcfa - $commande->montant_acompte_fcfa, 0, ',', ' ') }} FCFA
                    </p>
                </div>
            </div>

            @if(($commande->montant_total_fcfa - $commande->montant_acompte_fcfa) > 0)
